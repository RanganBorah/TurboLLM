    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {concepts.map((c, idx) => (
          <div
            key={idx}
            className="rounded-lg border border-slate-800 bg-slate-900 p-5 space-y-3 hover:border-slate-700 transition-colors"
          >
            <div className="flex items-center justify-between">
              <div className="p-2 rounded-md bg-slate-800">
                {c.icon}
              </div>
              <span className="text-[11px] text-slate-500 px-2 py-0.5 rounded bg-slate-950 border border-slate-800">
                {c.tag}
              </span>
            </div>
